...
- **Capability Mapping**: Map MCP tool names → Hermes capabilities
- **Auth Injection**: Inject PASETO capability tokens into MCP calls
- **Protocol Translation**: JSON-RPC 2.0 ↔ ACP/Tool interface
- **Streaming Support**: MCP streaming → ACP streaming
- **Health Monitoring**: Ping/pong, reconnection logic
- **Rate Limiting**: Per-server, per-tool limits
...
| `HealthCheck` | All Registries | Aggregate health status |
