...
| **Resilient by Default** | Idempotent ingestion, DLQ, retries, graceful degradation |
| **Consistency-Aware** | Snapshot isolation for RAG reads during incremental sync |
...
### 5.5 Ingestion Idempotency (C-01)
...
### 5.6 Ingestion Backpressure & Rate Limiting (H-06)
...
### 7.4 Partial Document Update Strategy (H-01 Addendum)

```
Document Change Detected
    │
    ├─▶ Compute Section-Level Hashes (per heading/section)
    │
    ├─▶ Compare with Stored Section Hashes
    │
    ├─▶ For Changed Sections Only:
    │       ├─▶ Re-chunk affected sections
    │       ├─▶ Re-embed affected chunks
    │       ├─▶ Re-extract entities
    │       └─▶ Upsert new chunks (deterministic chunk_id)
    │
    ├─▶ For Unchanged Sections: Skip entirely
    │
    └─▶ Update Document content_hash + section_hashes
```
...
message KnowledgeChunk {
  string chunk_id = 1;                    // UUID v7 (deterministic: hash(ingestion_key + chunk_index))
...
message DocumentMetadata {
  ...
  // Section hashes for partial updates
  map<string, string> section_hashes = 12;  // section_id -> hash
}
```
...
### 10.4 Sparse Vector Index Configuration (H-05)

```yaml
# Qdrant sparse vector configuration for SPLADE/BM25
collections:
  knowledge_chunks:
    vectors:
      dense:
...
User Query
    ...
    ├─▶ Post-Processing
    │       │
    │       ├─▶ Citation Verification (C-02)
    │       ├─▶ Hallucination Check vs Evidence
    │       └─▶ Source linking
    │
    ▼
...
### 11.2 Multi-Hop Retrieval Implementation (H-02)
...
message RAGConfig {
  ...
  bool enable_citation_verification = 13; // true (C-02)
  
  // Citations
  CitationStyle citation_style = 14;      // INLINE, FOOTNOTE, BRACKET
  bool include_source_links = 15;         // true
}
```

### 11.4 Citation Verification (C-02)

```
For Each Citation in Generated Answer:
    │
    ├─▶ Retrieve Cited Chunk (by chunk_id)
    │
    ├─▶ Extract Answer Span (sentence containing citation)
    │
    ...
    │
    └─▶ Attach Verification Result to Citation Metadata
```

### 11.5 Hallucination Check vs Evidence

```
Self-Consistency Check (3 samples):
    │
    ├─▶ Generate 3 independent answers (temperature=0.7)
    │
    ├─▶ Pairwise NLI between samples
    │       High contradiction → Low confidence
    │
    └─▶ Evidence Grounding Check:
            For each claim in answer:
                ├─▶ Find supporting chunks in context
                ├─▶ Compute entailment score
                └─▶ Unsupported claims → flag/remove
```

### 11.6 Retrieval Response
...
message RAGAnswer {
  ...
  // Verification results (C-02)
  repeated CitationVerification citation_verifications = 7;
  HallucinationCheckResult hallucination_check = 8;
}
...
message CitationVerification {
  string citation_id = 1;
  VerificationResult result = 2;          // VERIFIED, FALSE, WEAK
  float entailment_score = 3;             // 0-1
  string evidence_span = 4;
}

message HallucinationCheckResult {
  float self_consistency_score = 1;       // 0-1
  float evidence_grounding_score = 2;     // 0-1
  repeated UnsupportedClaim unsupported_claims = 3;
}
```
...
Chunks (batched)
    ...
    │
    ├─▶ Entity Linking & Disambiguation (H-01)
    │       │
    │       ├─▶ Candidate Generation: Fuzzy match (name + type) → top 10
    │       ├─▶ Context Scoring:
    │       │       ├─▶ Chunk context similarity (embedding cosine)
    │       │       ├─▶ Type compatibility
    │       │       └─▶ Co-occurrence frequency (tenant-wide)
    │       ├─▶ Scoring: weighted sum (context 0.5 + type 0.3 + co-occurrence 0.2)
    │       ├─▶ Thresholds:
    │       │       > 0.85 → Auto-merge
...
### 12.4 Graph Population DLQ & Idempotency (C-03, H-01)

```yaml
graph_population:
  stages:
    - name: entity_extraction
      dlq_topic: hermes.dlq.knowledge.entity_extraction
      max_retries: 3
      backoff: exponential
      base_delay_ms: 1000
      
    - name: relation_extraction
      dlq_topic: hermes.dlq.knowledge.relation_extraction
      max_retries: 3
      backoff: exponential
      base_delay_ms: 1000
      
    - name: entity_linking
      dlq_topic: hermes.dlq.knowledge.entity_linking
      max_retries: 2
      backoff: exponential
      base_delay_ms: 2000

  idempotency:
    # Kuzu MERGE with event_id as idempotency key
    entity_upsert: |
...
message SourceAttribution {
  ...
  CredibilityScore credibility = 10;    // Source reliability (H-03)
}
```

### 13.2 Source Credibility Model (H-03)
...
```
overall_score = 0.4 * domain_authority + 0.2 * recency + 0.2 * user_feedback + 0.2 * verification_status
```
...
| **Credibility Threshold** | Filter sources below configurable threshold (default: 0.3) |
...
Source Change Detected
    ...
    │       │       │
    │       │       ├─▶ Re-chunk (partial update for large docs)
    │       │       │
    │       │       ├─▶ Re-embed
    │       │       │
    │       │       ├─▶ Re-extract Entities
...
### 14.3 Document Deletion Cascading & Verification (H-07)
...
| **Deletion Verification** | < 60 seconds |
...
### 15.3 Snapshot Isolation for RAG Reads (C-04)
...
### 15.4 Graph Index Management (Kuzu)
...
### 15.5 Embedding Model Migration
...
service KnowledgeService {
  ...
  // Verification (C-02)
  rpc VerifyCitations(VerifyCitationsRequest) returns (VerifyCitationsResponse);
  
  // Multi-hop (H-02)
  rpc DecomposeQuery(DecomposeQueryRequest) returns (DecomposeQueryResponse);
}
```
...
// Citation Verification (C-02)
message VerifyCitationsRequest {
  string answer_id = 1;
  repeated Citation citations = 2;
  repeated string context_chunks = 3;     // Context for NLI
}

message VerifyCitationsResponse {
  repeated CitationVerification results = 1;
}

// Multi-hop Query Decomposition (H-02)
message DecomposeQueryRequest {
  string query = 1;
  string tenant_id = 2;
  int32 max_sub_queries = 3;              // Default: 5
}

message DecomposeQueryResponse {
  repeated SubQuery sub_queries = 1;
  QueryPlan plan = 2;                     // Execution order, dependencies
}

message SubQuery {
  string sub_query_id = 1;
  string query_text = 2;
  repeated string depends_on = 3;         // Other sub_query_ids
  SearchFilters filters = 4;
}

message QueryPlan {
  repeated string execution_order = 1;    // sub_query_ids in order
...
| `v1.hermes.knowledge.document.deletion_verified` | `hermes.knowledge.document.deletion_verified` | Document ID + verification |
...
| `v1.hermes.knowledge.graph.dlq` | `hermes.dlq.knowledge.*` | Failed extraction + error |
