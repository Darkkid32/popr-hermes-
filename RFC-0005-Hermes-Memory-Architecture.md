...
message WorkingMemory {
  string session_id = 1;              // Conversation session
  string tenant_id = 2;
  string workspace_id = 3;
  string user_id = 4;
  
  // Conversation context
  repeated ConversationTurn recent_turns = 5;      // Last 20 turns
  ConversationSummary summary = 6;                  // Compressed history
  
  // Active workflow state
  string active_workflow_id = 7;
  WorkflowState workflow_state = 8;
  
  // Agent scratchpads (ephemeral)
  ...
  int32 ttl_seconds = 13;  // Default: 300 (5 min after last access)
}
...
message AgentScratchpad {
  string agent_id = 1;
  bytes data = 2;               // Opaque binary (agent-specific)
...
- **On Eviction**: Flush to Episodic Memory (consolidation trigger)
...
message EpisodicEvent {
  ...
  string correlation_id = 2;              // Conversation/session ID
  ...
  // Source
  string source_module = 8;               // conversation, agent-runtime, workflow, etc.
  string source_agent_id = 9;             // If agent-generated
  
  // Event type
  string event_type = 10;                 // e.g., conversation.turn, tool.call, workflow.step
  bytes payload = 11;                     // Protobuf Any (domain-specific)
  
  // Embeddings (computed async)
  repeated float vector_embedding = 12;   // 1536-dim (text-embedding-3-small)
  string embedding_model = 12;
  
  // Graph relations (extracted async)
  repeated EntityRelation entity_relations = 13;
  
  // Privacy
  PrivacyLevel privacy_level = 14;        // PUBLIC, PRIVATE, PII, SECRET
...
message SemanticMemoryEntry {
  string entry_id = 1;                    // UUID v7
  string tenant_id = 2;
  string workspace_id = 3;                // Optional domain scope
  
  // Content
  string content = 4;                     // Text, code, markdown
  string content_type = 5;                // text, code, markdown, doc
  map<string, string> metadata = 6;       // source, author, tags, version
  
  // Embeddings
  repeated float vector_embedding = 7;    // 1536-dim
  string embedding_model = 8;
  
  // Graph
  string graph_node_id = 9;               // Kuzu node ID
  repeated string related_concepts = 10;  // Concept IDs
  
  // Versioning
  string version = 9;                     // SemVer
  string previous_version_id = 10;
  string change_summary = 11;
  
  // Lifecycle
  MemoryState state = 11;                 // ACTIVE, DEPRECATED, ARCHIVED
  int64 created_at_us = 12;
  int64 updated_at_us = 13;
  int64 expires_at_us = 14;               // 0 = never
  
  // Quality
  float quality_score = 15;               // 0-1 (human + auto eval)
...
message ProceduralSkill {
  string skill_id = 1;                    // e.g., "code-review.v2", "deploy.k8s.v1"
  string tenant_id = 2;
  
  // Definition
  string name = 3;
  string description = 4;
  string category = 5;                    // coding, devops, analysis, etc.
  repeated string tags = 6;
  
  // Capability contract
  SkillInputSchema input_schema = 4;      // JSON Schema
  SkillOutputSchema output_schema = 5;    // JSON Schema
  
  // Implementation (versioned)
  string implementation_version = 5;      // SemVer
  SkillImplementation implementation = 6; // Code, workflow, or hybrid
  ...
  // Quality & Usage
  float success_rate = 6;                 // 0-1
  int32 invocation_count = 6;
  float avg_latency_ms = 7;
  float quality_score = 8;                // Human + auto eval
  
  // Lifecycle
  SkillState state = 8;                   // ACTIVE, DEPRECATED, ARCHIVED
...
| `InvokeSkill(skill_id, input)` | Variable (delegates to agent) | N/A |
...
- **Rollback**: Instant via version pinning
...
message UnifiedSearchRequest {
  string tenant_id = 1;
  string query = 2;
  SearchScope scope = 3;        // ALL, EPISODIC, SEMANTIC, PROCEDURAL
...
| **Episodic→Semantic** | Nightly (02:00 UTC) | Episodic events (24h window) | Semantic entries (facts, patterns) | Daily |
| **Pattern Mining** | Weekly (Sunday 03:00) | Semantic entries | Procedural skill candidates | Weekly |
| **Skill Validation** | On candidate creation | Skill candidate | Validated skill or rejection | Async |
| **Quality Re-eval** | Monthly | All semantic entries | Updated quality scores | Monthly |

### 10.2 Consolidation Details

#### Episodic → Semantic (Nightly)
1. **Window**: Last 24h of episodic events per tenant
...
3. **Deduplication**: Merge with existing semantic entries (embedding similarity > 0.95)
4. **Quality Scoring**: Auto-eval (0-1) based on source credibility, recency, uniqueness
5. **Persistence**: Upsert to semantic memory with provenance links

#### Pattern Mining → Procedural (Weekly)
...
5. **Registration**: If valid, register as Procedural Skill (v0.1.0)
...
service MemoryService {
  ...
  // Context Assembly (for RFC-0004 Gateway)
...
message AssembleContextRequest {
  ...
  string intent = 3;                    // Parsed intent (optional)
  TokenBudget budget = 4;               // Total token budget
...
### 15.3 Graph Population
- **Event-driven**: Episodic events → entity/relation extraction → Kuzu upsert
- **Consolidation**: Semantic entries → concept nodes + relations
- **Skill Registration**: Skill → graph nodes for dependencies
...
| **Eventual Semantic** | Tenant | Consolidation pipeline (< 24h) |
...
```protobuf
// Core retrieval for context assembly (RFC-0004)
rpc AssembleContext(AssembleContextRequest) returns (AssembleContextResponse);
...
// Context Assembly (Gateway ↔ Memory)
rpc AssembleContext(AssembleContextRequest) returns (AssembleContextResponse);
