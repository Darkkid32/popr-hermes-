...
```protobuf
message Session {
  string session_id = 1;                  // UUID v7
  string user_id = 2;
  string tenant_id = 3;
  
  // Authentication
  AuthenticationMethod auth_method = 4;
  string auth_claims_hash = 5;            // Hash of original claims
  
  // Tokens
  string access_token_jti = 6;            // For revocation
  string refresh_token_hash = 7;          // Hashed for storage
  
  // Device
  string device_fingerprint = 8;
  string ip_address = 9;
  string user_agent = 10;
  string geolocation = 11;
  
  // Security
  SessionStatus status = 12;              // ACTIVE, REVOKED, EXPIRED, COMPROMISED
  int64 created_at_us = 13;
  int64 last_activity_us = 14;
  int64 expires_at_us = 15;
  int64 absolute_expires_at_us = 16;      // Hard limit (24h)
  
  // Risk
  RiskLevel risk_level = 17;              // LOW, MEDIUM, HIGH, CRITICAL
  repeated RiskSignal risk_signals = 18;  // Impossible travel, new device, etc.
}
```
...
│             │     │             │     │             │     │             │
│ - Validate  │     │ - Sliding   │     │ - Monitor   │     │ - Revoke    │
│   Creds     │     │   Window    │     │   Inactivity│     │ - Cleanup   │
│ - Issue     │     │ - Refresh   │     │ - Risk Check│     │ - Audit     │
│   Tokens    │     │ - Risk      │     │             │     │             │
│ - Device FP │     │   Monitor   │     │             │     │             │
└─────────────┘     └─────────────┘     └─────────────┘     └─────────────┘
       │                   │                   │                   │
...
### 20.3 Session Security Controls

| Control | Implementation |
|---------|----------------|
| **Token Binding** | Access token bound to device fingerprint + IP prefix (/24) |
| **Concurrent Sessions** | Limit: 20 per user; oldest revoked on new login |
| **Absolute Timeout** | 24 hours hard limit (regardless of activity) |
| **Idle Timeout** | 30 minutes (configurable per tenant) |
| **Risk-Based Step-Up** | Impossible travel → MFA challenge; new device → email confirmation |
| **Revocation** | Immediate via Redis blocklist (JTI); propagated to all services via NATS |
| **Session Hijacking Detection** | Token replay detection; geo-velocity analysis |

### 20.4 Session Risk Scoring Algorithm (H-02)

```rego
# Session risk scoring based on geo-velocity and behavioral analysis
package hermes.session

# Risk signals
risk_signal["impossible_travel"] {
  prev = input.previous_location
  curr = input.current_location
  time_diff_hours = (input.current_time - prev.timestamp) / 3600
  distance_km = geo.distance(prev.lat, prev.lon, curr.lat, curr.lon)
  velocity_kmh = distance_km / time_diff_hours
  velocity_kmh > 800  # Supersonic = impossible
}

risk_signal["new_device"] {
  not device_known(input.device_fingerprint, input.user_id)
}

risk_signal["new_location"] {
  not location_known(input.geolocation, input.user_id)
}

risk_signal["credential_stuffing"] {
  failed_attempts > 5
...
# Step-up authentication required for medium+
step_up_required {
...
### 21.1 Gateway Security Controls

| Control | Implementation |
|---------|----------------|
| **TLS Termination** | TLS 1.3 only; HSTS; certificate transparency monitoring |
| **Authentication** | JWT validation (JWKS cached); API key lookup; mTLS for service clients |
| **Rate Limiting** | Token bucket per tier (Anonymous: 10/min, User: 120/min, Dev: 600/min, Admin: 3000/min, Service: 10000/min) |
| **Request Validation** | JSON Schema validation; size limits (16 MB); content-type enforcement |
| **Response Headers** | CSP, HSTS, X-Frame-Options, Referrer-Policy, Permissions-Policy |
| **CORS** | Strict allowlist per tenant; no wildcards |
| **Request Signing** | Optional HMAC-SHA256 for high-value APIs |

### 21.2 Input Validation

| Vector | Protection |
|--------|------------|
| **JSON Body** | JSON Schema validation (strict); max depth 32; max keys 1000 |
| **Query Params** | Allowlist + type coercion; max 50 params |
| **Headers** | Allowlist; strip hop-by-hop headers; sanitize |
| **Path Params** | UUID format validation; regex for custom IDs |
| **File Upload** | MIME type verification; magic bytes; size limit; malware scan |

### 21.3 Output Encoding

| Context | Encoding |
|---------|----------|
| **JSON** | Native (Content-Type: application/json; charset=utf-8) |
| **SSE** | Native (text/event-stream) |
| **File Download** | Content-Disposition: attachment; X-Content-Type-Options: nosniff |

---

## 22. Event Bus Security (NATS)

### 22.1 NATS Security Model

| Layer | Implementation |
|-------|----------------|
| **Transport** | TLS 1.3 with SPIFFE certificates (mTLS) |
| **Authentication** | SPIFFE SVID + NATS nkeys (dual) |
| **Authorization** | NATS Accounts per tenant + per-subject permissions |
| **Encryption** | Payload encryption for sensitive subjects (Vault Transit) |
| **Audit** | All publish/subscribe logged to audit stream |

### 22.2 NATS Account Structure
...
System Account (admin)
    ...
    │   ├── Permissions:
    │   │   ├── Publish: hermes.acme.>
    │   │   ├── Subscribe: hermes.acme.>
    │   │   └── Max Payload: 16 MB
    │   │
    │   ├── Stream: hermes.acme.audit (retention: 7 years)
    │   ├── Stream: hermes.acme.events (retention: 90 days)
    │   └── Consumer: acme-gateway, acme-core, acme-memory, ...
    │
    ├── Tenant Account: beta
    │   └── (isolated)
    │
    └── System Streams (shared)
        ├── hermes.system.health
        ├── hermes.system.config
        └── hermes.audit.global
```

### 22.3 Subject-Level Authorization (H-01 - Completed)
...
// Capability Token (enhanced with delegation)
message IssueCapabilityTokenRequest {
  string agent_id = 1;
  repeated Capability capabilities = 2;
  int32 ttl_seconds = 3;
  repeated string audience = 4;
  repeated string delegation_chain = 5;  // Optional parent delegation
  int32 max_delegation_depth = 6;        // Default: 3
}
...
message CapabilityToken {
  ...
  int32 delegation_depth = 4; // Current depth
  int32 max_delegation_depth = 5;
}
...
### 28.4 Zero Trust Network (Mermaid)
...
- [ ] **C-01**: Capability delegation chain verification in PASETO token + PDP validation
- [ ] **C-02**: Per-tenant SPIRE trust domains for enterprise isolation
- [ ] **C-03**: Audit transparency log verifier with daily Merkle root verification + alerting
- [ ] **C-04**: Push-based PDP cache invalidation via NATS events for token/capability revocation
- [ ] **C-05**: Maximum delegation depth limit (3) in capability token and PDP

### 29.4 High-Priority Review Items Addressed (H-01 to H-07)
- [ ] **H-01**: Complete NATS audit stream subscribe policy
- [ ] **H-02**: Session risk scoring algorithm (geo-velocity formula + thresholds)
- [ ] **H-03**: Network egress enforcement point for `network_egress` capability constraint
- [ ] **H-04**: Property-based testing framework for all Cedar/Rego policies
- [ ] **H-05**: Root CA rotation ceremony documentation (quorum, HSM, witnessing, key splitting)
- [ ] **H-06**: Cross-RFC contract tests for Security↔Gateway, Security↔Memory, Security↔Knowledge
- [ ] **H-07**: Capability token cache invalidation propagation to PDP decision cache
