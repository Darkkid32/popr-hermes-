  ...
  if (variant === 'compact') {
    return (
      <div
        ...
        onClick={onClick}
      >
        <span style={{ fontSize: 'var(--text-display-sm)', color: tool.serverColor }}>
          {tool.serverIcon}
        </span>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-2)', marginTop: 'var(--spacing-1)' }}>
            <ProviderBadge simpleProvider={{ name: tool.serverName, status: 'connected', icon: tool.serverIcon, iconColor: tool.serverColor }} size="sm" />
          </div>
        </div>
      </div>
    )
    ...
    return (
      <Card variant="elevated">
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--spacing-3)', marginBottom: 'var(--spacing-3)' }}>
          <span style={{ fontSize: 'var(--text-display-md)', color: tool.serverColor }}>
            {tool.serverIcon}
          </span>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-2)' }}>
              <ProviderBadge simpleProvider={{ name: tool.serverName, status: 'connected', icon: tool.serverIcon, iconColor: tool.serverColor }} size="sm" />
              <Badge variant="info" size="sm">MCP Tool</Badge>
            </div>
          </div>
        </div>
